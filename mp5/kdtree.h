...
class KDTree
{
  ...
  private:
     ...
     */
     KDTreeNode* buildTree(vector<Point<Dim>> & Points, int begin, int end, int dim);
     void quickSelect(vector<Point<Dim>> & Points, int begin, int end, int k, int dim);
     int partition(vector<Point<Dim>> & Points, int begin, int end, int pivotIDX, int dim);
     KDTreeNode* _copy(KDTreeNode * subroot);
     void _destroy(KDTreeNode * subroot);
     Point<Dim>findNearestNeighbor_(const KDTreeNode * subroot, const Point<Dim> target, int dim,  Point<Dim> childResult, Point<Dim> currentBest) const;
     //std::stack<KDTreeNode*> s_;
     double distance(const Point<Dim>& target, const Point<Dim>& current) const;
