...
class Graph {
  ...
  public:
    ...
    // Graph algorithm:
    std::list<std::string> shortestPathBFS(const std::string start, const std::string end);
    std::list<std::string> shortestPath(const std::string start, const std::string end);
    void UpdatingPQ(std::queue<std::string> &pq,  std::unordered_map<string, bool> &visited,  std::unordered_map<string, int> &dist);
