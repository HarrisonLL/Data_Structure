...
Animation FloodFilledImage::animate(unsigned frameInterval) const {
  ...
  PNG imageCopy = PNG(image_);
  //animation.addFrame(imageCopy);

  for (int i = 0; i < (int)trav_.size(); i++) {
    unsigned itCount = 0;
    ImageTraversal *trav = trav_[i];
    for (auto it = trav -> begin(); it != trav -> end(); ++it) {
      if (itCount % frameInterval == 0) {
        animation.addFrame(imageCopy);
        //itCount = 0;
      }
      HSLAPixel &pixel0 = imageCopy.getPixel((*it).x, (*it).y);
      pixel0 = color_[i]-> getColor((*it).x,(*it).y);
      itCount++;
    //std::cout << "######################" << std::endl;
    }
